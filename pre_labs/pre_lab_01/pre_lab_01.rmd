...
**Answer: 79366 observations and 9 variables**
...
In the space below, write the name of the course from the first row of the "name" column. **Answer: Directed Research**
...
Describe what you think each line of this code is doing below, based on the result: I think the first line is telling r to open "umd courses" and to be continuous with the I\> pipe function, the second line of code uses the function group_by and tells it to do so by term and to keep going with the pipe, the third line summarizes in another column, and within that column is the type of summarizing we want to do (in this case, a count) "count_classes" in the next line is the title of the column, not the function which is "=n" with the circle.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: now the tibble is reordered, this time starting with the highest number in the "count" table, descending**
...
**Answer: departments were added as a column**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: another column for total seats was added**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: there were two columns added for mean and median seats.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted?: a new column was added for the department, and now the table is sorted with business management at the top.
...
1.  Which are the most frequently taught classes across semesters?

2.  Which classes fill up completely/which stay empty the longest?

3.  Which classes are taught throughout the year? Which are exclusive to one kind of term?
